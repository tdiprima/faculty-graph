...
VALID_ROW = ("bmi-001", "Ada Lovelace", "BMI", "0000-0002-1825-0097", "ada@example.edu")
...
def test_valid_seed_loads(seed_file):
    ...
    assert faculty[0]["faculty_id"] == "bmi-001"
...
def test_blank_orcid_is_allowed(seed_file):
    ...
    faculty = load_seed_faculty(seed_file([("bmi-001", "Ada", "BMI", "", "a@x.edu")]))

    assert faculty[0]["orcid"] == ""


def test_surrounding_whitespace_is_stripped(seed_file):
    rows = [("  bmi-001 ", " Ada Lovelace ", "BMI", "  ", "ada@example.edu")]
...
def test_multiple_rows_preserve_order(seed_file):
    rows = [VALID_ROW, ("bmi-002", "Grace Hopper", "BMI", "", "g@x.edu")]

    faculty = load_seed_faculty(seed_file(rows))

    assert [record["faculty_id"] for record in faculty] == ["bmi-001", "bmi-002"]
...
def test_duplicate_faculty_id_is_rejected(seed_file):
    ...
    rows = [VALID_ROW, ("bmi-001", "Grace Hopper", "BMI", "", "g@x.edu")]

    with pytest.raises(SeedDataError, match="duplicate faculty_id 'bmi-001'"):
        load_seed_faculty(seed_file(rows))


def test_error_message_names_the_offending_line(seed_file):
    rows = [VALID_ROW, ("bmi-002", "Grace", "BMI", "", "g@x.edu"), ("", "Alan", "BMI", "", "a@x.edu")]
...
def test_oversized_seed_list_loads(seed_file):
    ...
    rows = [(f"bmi-{n:05d}", f"Person {n}", "BMI", "", f"p{n}@x.edu") for n in range(5000)]
